// src/app/agent/dashboard/page.tsx - COMPLETE AGENT DASHBOARD
'use client';
...
import { 
  ...
  BarChart3, FileText, MessageSquare, UserCheck,
  Target, Briefcase, Star, LogOut
} from 'lucide-react';
...
const useRecentViewings = () => {
    ...
    try {
      ...
      setViewings([
        ...
        },
        {
          ...
          client_phone: '+267 72 234 567',
          scheduled_date: new Date(Date.now() + 3 * 24 * 60 * 60 * 1000).toISOString(),
          status: 'pending'
        }
      ]);
    } catch (err) {
      console.error('Error fetching recent viewings:', err);
  ...
  return { viewings, loading, error, refetch: fetchViewings };
};
...
const ViewingItem = ({ viewing }: { viewing: Viewing }) => {
  ...
  const scheduledDate = new Date(viewing.scheduled_date).toLocaleDateString('en-US', {
    month: 'short',
    day: 'numeric',
    hour: '2-digit',
    minute: '2-digit'
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-xl p-4 hover:shadow-sm transition-all">
      <div className="flex items-center justify-between">
        <div className="flex-1 min-w-0">
          <h3 className="font-medium text-gray-900 text-sm">{viewing.property_title}</h3>
          <p className="text-gray-600 text-xs mt-0.5">With {viewing.client_name}</p>
          <p className="text-gray-500 text-xs mt-1">{scheduledDate}</p>
          <p className="text-blue-600 text-xs mt-1">{viewing.client_phone}</p>
        </div>
        <div className={`px-2 py-1 rounded-full text-xs font-medium capitalize flex-shrink-0 ${getStatusColor(viewing.status)}`}>
          {viewing.status}
        </div>
      </div>
    </div>
...
export default function AgentDashboard() {
  ...
  const { properties, loading: propertiesLoading, error: propertiesError, refetch: refetchProperties } = useAgentProperties();
  const { stats, loading: statsLoading, error: statsError, refetch: refetchStats } = useDashboardStats();
  const { viewings, loading: viewingsLoading, error: viewingsError, refetch: refetchViewings } = useRecentViewings();
    ...
    <div className="min-h-screen bg-gray-50 pb-20 safe-area-inset">
      ...
      <main className="px-4 py-6 space-y-6">
          ...
          {statsError ? (
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <StatCard
                ...
                value={`P${stats.monthly_commission.toLocaleString()}`}
                icon={DollarSign}
                color="green"
        ...
        <section>
          <div className="flex items-center justify-between mb-4">
            <div>
              <h2 className="text-lg font-semibold text-gray-900">Recent Viewings</h2>
              <p className="text-gray-600 text-sm">Property tours</p>
            </div>
            <button 
          ...
          {viewingsError ? (
            <ErrorBoundary message={viewingsError} onRetry={refetchViewings} />
          ) : viewingsLoading ? (
      ...
      <nav className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 safe-area-bottom">
        <div className="px-4 py-2">
          <div className="flex justify-around items-center">
            {[
              { icon: Home, label: 'Dashboard', active: true, href: '/agent/dashboard' },
              { icon: Building2, label: 'Listings', href: '/agent/properties' },
